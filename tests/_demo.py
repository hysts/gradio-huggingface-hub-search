"""Test-only Gradio app for UI tests.

This module is underscore-prefixed so pytest does not collect it.
It mirrors the configurations exercised by the test suite without
coupling tests to the public demo/app.py.
"""
...
def _on_select(value: dict | None) -> tuple[str, str, str]:
    if not value:
        return "", "", ""
    return (
        value.get("id") or "",
